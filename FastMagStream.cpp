...
void ShowStartupError(const std::string &message) {
  MessageBoxA(nullptr, message.c_str(), "FastMagStream Startup Error",
              MB_OK | MB_ICONERROR);
}
} // namespace

void CaptureScreenAndDisplay(HWND windowHandle, const AppConfig &config) {
  const int captureWidth = static_cast<int>(
      static_cast<double>(config.display_width) / config.zoom_factor);
  const int captureHeight = static_cast<int>(
      static_cast<double>(config.display_height) / config.zoom_factor);
  const auto frameDelay =
      std::chrono::duration<double, std::milli>(ComputeFrameDelayMs(config));
  ...
  const int clipCropX = (cropX < 0) ? 0
                                    : (cropX + captureWidth > desktopWidth
                                           ? desktopWidth - captureWidth
                                           : cropX);
  const int clipCropY = (cropY < 0) ? 0
                                    : (cropY + captureHeight > desktopHeight
                                           ? desktopHeight - captureHeight
                                           : cropY);
  ...
  HBITMAP hDib =
      CreateDIBSection(hMemoryDC, &bmi, DIB_RGB_COLORS, &pDibBits, nullptr, 0);
  if (!hDib || !pDibBits) {
    DeleteDC(hMemoryDC);
    pStaging->Release();
    pDuplication->Release();
  ...
  auto copyFrameToDib = [&]() -> int {
    ...
    hr = pResource->QueryInterface(__uuidof(ID3D11Texture2D),
                                   reinterpret_cast<void **>(&pDesktopTexture));
    pResource->Release();
    if (FAILED(hr) || !pDesktopTexture) {
    ...
    const char *pSrc = static_cast<const char *>(mapped.pData) +
                       clipCropY * mapped.RowPitch + clipCropX * 4;
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE, PWSTR, int nCmdShow) {
  ...
  if (!RegisterClassW(&wc)) {
    ShowStartupError("Failed to register window class.");
    return 1;
  }

  HWND hwnd =
      CreateWindowW(L"ScreenCaptureWindowClass", L"FastMagStream",
                    WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MAXIMIZEBOX,
                    CW_USEDEFAULT, CW_USEDEFAULT, config.display_width,
                    config.display_height, NULL, NULL, hInstance, NULL);

  if (!hwnd) {
    ShowStartupError("Failed to create window.");
    return 1;
  }
  ...
  std::thread captureThread(CaptureScreenAndDisplay, hwnd, std::cref(config));
